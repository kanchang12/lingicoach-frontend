import os, uuid, time, httpx, hmac, hashlib, json as _json, re as _re
...
def verify_google_play_subscription(package_name, subscription_id, purchase_token):
    ...
    try:
        ...
        if resp.get("purchaseState", -1) != 0:
            return False, None, "Purchase not active"
        return True, resp.get("expiryTimeMillis"), None
...
def sb(method, path, body=None, token=None, upsert=False):
    ...
    r = httpx.request(method, f"{SUPABASE_URL}/rest/v1{path}", headers=hdrs, json=body, timeout=10)
    return r.json() if r.text else {}

def sb_auth(method, path, body=None):
    hdrs = {"apikey": SUPABASE_ANON, "Content-Type": "application/json"}
    r    = httpx.request(method, f"{SUPABASE_URL}/auth/v1{path}", headers=hdrs, json=body, timeout=10)
...
def set_premium_with_expiry(user_id, plan, expiry_time_millis=None):
    ...
    sb("POST", "/user_stats", {"user_id": user_id, "is_premium": True, "premium_expires_at": expiry}, upsert=True)

def is_premium(user_id):
    s = get_stats(user_id)
    if not s.get("is_premium"): return False
    exp = s.get("premium_expires_at")
    if exp and datetime.fromisoformat(exp) < datetime.now(timezone.utc): return False
    return True

def rate_ok(ip):
    now = time.time()
    rate_limits[ip] = [t for t in rate_limits[ip] if now - t < 60]
    if len(rate_limits[ip]) >= 40: return False
...
def signup():
    ...
    if result.get("user"):
        sb("POST", "/user_stats", {"user_id": result["user"]["id"], "streak": 0})
...
def forgot():
    sb_auth("POST", "/recover", {"email": request.json.get("email")})
...
def user_stats():
    ...
    pm    = is_premium(uid)
    return jsonify({
        "streak": stats.get("streak", 0), "done": len(prog),
        "progress": list(prog), "is_premium": pm,
...
def set_language():
    ...
    if lang not in LANGUAGES: return jsonify({"error": "Unsupported language"}), 400
...
def save_progress():
    ...
    if sid: mark_done(uid, sid)
    return jsonify({"ok": True})

@app.route("/session/start", methods=["POST"])
@require_auth
def start_session():
    if not rate_ok(request.remote_addr): return jsonify({"error": "Too many requests"}), 429
    data      = request.json
    lang_code = data.get("native_language_code", "")
    if lang_code not in LANGUAGES: return jsonify({"error": "Unsupported language"}), 400
    clean_sessions()
    ...
    try:
        ...
        return jsonify({"session_id": sid, "message": clean_for_speech(resp.text)})
    except Exception as e:
...
def chat_message():
    ...
    if time.time() - s["last"] > SESSION_TTL:
        del sessions[sid]; return jsonify({"error": "Session expired"}), 410
    msg = (data.get("message") or "").strip()
    if not msg or len(msg) > 1000: return jsonify({"error": "Invalid message"}), 400
    try:
...
def translate():
    ...
    try:
        resp = gemini.models.generate_content(
            model=MODEL, contents=data["text"],
            config=types.GenerateContentConfig(system_instruction=translate_prompt(from_lang, to_lang))
        )
        return jsonify({"translation": resp.text.strip()})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

# ── NEW: Scenario demo — generates model conversation for Listen → Learn ───
...
def scenario_demo():
    ...
    prompt = (
        ...
        f"- key_phrases: 5 most useful expressions lifted directly from the conversation\n"
...
def play_subscribe():
    ...
    if not valid: return jsonify({"error": error or "Subscription verification failed"}), 402
...
def log_event(event_type, detail, user_id=None):
    execution_log.append({
        ...
        "type": event_type, "detail": detail, "user": user_id or "anon"
    })
    if len(execution_log) > 200: execution_log.pop(0)

@app.before_request
def track_request():
    p, m = request.path, request.method
    if p == "/session/start"          and m == "POST": api_counters["sessions_started"] += 1; api_counters["gemini_calls"] += 1
    elif p == "/session/chat"         and m == "POST": api_counters["gemini_calls"] += 1
    elif p == "/translate"            and m == "POST": api_counters["translate_calls"] += 1; api_counters["gemini_calls"] += 1
    elif p == "/scenario/demo"        and m == "POST": api_counters["gemini_calls"] += 1
    elif p == "/payment/play-subscribe" and m == "POST": api_counters["play_subscriptions"] += 1

@app.after_request
def log_response(resp):
    if request.method in ("POST", "DELETE") and any(
        request.path.startswith(p) for p in ["/session", "/translate", "/auth", "/payment", "/user", "/scenario"]
    ):
        status = "OK" if resp.status_code < 400 else str(resp.status_code)
        uid = None
        if hasattr(request, "user"):
            uid = request.user.get("email") or request.user.get("id","")[:8]
        log_event(request.path, status, uid)
        if resp.status_code >= 400: api_counters["errors"] += 1
    api_counters["sessions_active"] = len(sessions)
    return resp

@app.route("/admin")
def admin_dashboard():
    if request.args.get("key", "") != ADMIN_KEY: return "Unauthorised", 401
    return render_template("admin.html", key=request.args.get("key"))

@app.route("/admin/data")
def admin_data():
    if request.args.get("key", "") != ADMIN_KEY: return jsonify({"error": "Unauthorised"}), 401
    total_users = premium_users = total_done = 0
    try:
    ...
    try:
        ...
        if isinstance(prog, list): total_done = len(prog)
    except Exception: pass
    return jsonify({
        "counters": {**api_counters, "total_users": total_users,
                     "premium_users": premium_users, "scenarios_completed": total_done},
        "sessions": [{"id": k[:8]+"...", "lang": v.get("lang","?"),
                      "msgs": v.get("count",0), "age_s": int(time.time() - v["last"])}
